"""
1D Bar - MMS cubic Solution 

Manufactured solution : u(x) = x^2*(L-x)/L^2
Source term           : f(x) = E*(6x - 2L)/L^2
Neumann BC at x=L     : sigma(L) = E*u'(L) = -E/L
"""
...
def _import_sofa_run():
    for module_name in ("sofa_bar_mms", "Sofa_bar_mms", "sofa_bar_MMS"):
        try:
            ...
            continue
    raise ImportError(
        "Cannot import sofaRun. "
        "Make sure sofa_bar_mms.py is in the same directory."
    )
...
def l2_error_discrete(u_num, u_ref):
    ...
    if len(u_num) != len(u_ref):
        raise ValueError(
            f"Incompatible dimensions: u_num={len(u_num)}, u_ref={len(u_ref)}."
        )
...
if __name__ == "__main__":
    ...
    runner = FreeFemRunner("freefem_bar_mms.edp")
    ...
    for nx in nx_list:
        ...
        x_ff   = exports["xcoords"]   # coordonnees des noeuds
        u_ff   = exports["u[]"]       # solution EF aux noeuds
        errors = exports["errors"]    # err_nodal, err_L2, err_H1
        ...
        # Interpoler SOFA sur les noeuds FreeFEM pour comparaison equitable
        ...
        print(f"         FF  u(0)={u_ff[0]:.2e}  u(L)={u_ff[-1]:.2e}"
              f"  u_max={np.max(u_ff):.6f} @ x={x_ff[np.argmax(u_ff)]:.3f}")
        print(f"         SOFA u(0)={u_sofa[0]:.2e}  u(L)={u_sofa[-1]:.2e}"
              f"  u_max={np.max(u_sofa):.6f} @ x={x_sofa[np.argmax(u_sofa)]:.3f}")

        # Condition de Neumann SOFA 
        du_sofa_L = (u_sofa[-1] - u_sofa[-2]) / (x_sofa[-1] - x_sofa[-2])
        print(f"         SOFA sigma(L)={E * du_sofa_L:.4f}  "
              f"(exact={-E/L:.4f})")
        print()
    ...
    ax1.plot(x_fine, u_ex_fine,
             label="Exact  u(x) = x²(L-x)/L²",
             color="black", linestyle="--", linewidth=1.8)
    ax1.plot(x_fine, u_ff_fine,
             label=f"FreeFEM  (nx={nx_list[-1]})",
             marker="o", markersize=4, linestyle="none", color="steelblue")
    ax1.plot(x_fine, u_sofa_fine,
             label=f"SOFA     (nx={nx_list[-1]})",
             marker="x", markersize=5, linestyle="none", color="tomato")
    ax1.set_xlabel("x")
    ax1.set_ylabel("Déplacement u(x)")
    ax1.set_title("Barre 1D MMS — Comparaison des déplacements\n"
                  "u(x) = x²(L-x)/L²")
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    fig1.tight_layout()
    fig1.savefig("results/plot_displacement.png", dpi=150)
    plt.close(fig1)
    print("\nSaved: results/plot_displacement.png")

    #  Plot 2 : Convergence L2 log-log 
    fig2, ax2 = plt.subplots(figsize=(6, 5))

    ax2.loglog(h_arr, err_nodal_arr,
               marker="s", linestyle=":", color="gray",
               label=f"FreeFEM nodal  (rate={rate_nodal:.2f})  superconvergence")
    ...
    ax2.loglog(h_arr, ref2, linestyle="--", color="gray",  alpha=0.6, label="O(h²) ref")
    ax2.loglog(h_arr, ref1, linestyle="--", color="lightblue", alpha=0.8, label="O(h¹) ref")

    ax2.set_xlabel("h  (taille d'élément)")
    ax2.set_ylabel("Erreur")
    ax2.set_title("Convergence MMS — Barre 1D\n"
                  "Superconvergence nodale vs erreurs continues")
    ax2.legend(fontsize=8)
    ax2.grid(True, which="both", alpha=0.3)
    fig2.tight_layout()
    fig2.savefig("results/plot_convergence.png", dpi=150)
    plt.close(fig2)


    summary_path = "results/comparison_results.txt"
    with open(summary_path, "w") as f:
        f.write("  MMS Cubic — Résumé de convergence\n")
        f.write(f"  u_exact(x) = x^2*(L-x)/L^2   L={L}  E={E}\n")
        ...
        for nx, h, en, el, eh, es in zip(
            ...
            f.write(f"{nx:>6}  {h:>9.5f}  "
                    ...
                    f"{eh:>13.4e}  {es:>13.4e}\n")
        f.write("\n")
        f.write(f"Taux FreeFEM nodal  : {rate_nodal:+.3f}  (attendu ~ 0, superconvergence)\n")

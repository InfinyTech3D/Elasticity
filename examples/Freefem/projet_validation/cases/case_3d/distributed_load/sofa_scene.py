"""
3D Beam Simulation - Distributed Load on Top Face - P1 Tetrahedra
...
def consistent_traction_forces(nodes, top_faces, q): 
    N = len(nodes)
    F = np.zeros((N, 3))
...
def create_scene_args(rootNode, mesh_file, q, young_modulus, poisson_ratio, tol=1e-6):
    ...
    with rootNode.addChild('Beam') as Beam:
        ...
        top_mask = np.all(np.isclose(nodes[tris, 1], y_max, atol=tol), axis=1)
        top_faces = tris[top_mask]

        F_nodal = consistent_traction_forces(nodes, top_faces, q)
...
def sofaRun(mesh_file, q, young_modulus, poisson_ratio):
    ...
    out_path = os.path.join(RESULTS_DIR, "sofa_beam3d_distributed_results.txt")

"""
2D Beam Simulation - Distributed Load - P1 Triangles
...
Two regimes, selected via `mode` :
    - "plane_strain" : template="Vec3d" (z kept at 0), matches the validated
      MMS pattern.
    - "plane_stress" : template="Vec2d" (genuine 2D elasticity, no z dof),
      matches the incompressibility MMS study convention
      (dim="2d" -> Vec2d -> plane stress, dim="3d" -> Vec3d -> plane strain).

Both use the same LinearSmallStrainFEMForceField; only the template and the
resulting position/force array shapes change.
"""
import json
import os
import sys
...
def create_scene_args(rootNode, mesh_file, q, young_modulus, poisson_ratio, mode="plane_strain"):
    ...
    if mode == "plane_strain":
        # z kept at 0 -> eps_zz = 0 imposed -> plane strain
        template = "Vec3d"
        positions = np.column_stack([nodes, np.zeros(N)]).tolist()
        forces_list = [[F_nodal[i, 0], F_nodal[i, 1], 0.0] for i in range(N)]
    else:
        # genuine 2D elasticity, no z dof -> plane stress
        template = "Vec2d"
...
def sofaRun(mesh_file, q, young_modulus, poisson_ratio, mode="plane_strain"):
    ...
    if mode == "plane_strain":
        u = pos_final[:, :2] - pos0[:, :2]   # keep only ux, uy (drop the z=0 dof)
        x0 = pos0[:, :2]
    else:
        u = pos_final - pos0                 # already (N,2): ux, uy directly
        x0 = pos0

    os.makedirs(RESULTS_DIR, exist_ok=True)
    out_path = os.path.join(RESULTS_DIR, f"sofa_beam2d_distributed_{mode}_results.txt")

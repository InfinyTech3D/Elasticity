...
_a4, _b4 = 0.5854101966249685, 0.1381966011250105   # symmetric 4-point rule
_TET_QUADRATURE = {
    1: (np.array([[1/4, 1/4, 1/4]]),
        np.array([1/6])),
    4: (np.array([[_a4, _b4, _b4], [_b4, _a4, _b4],
                  [_b4, _b4, _a4], [_b4, _b4, _b4]]),
...
def tet_p1_rule(n_pts=1):
    """Element rule for P1 tetrahedra: 1-point (centroid) or 4-point Gauss.

    Shape gradients are constant per tetrahedron (linear element). Node
    order in `xe` is the canonical P1 ordering — corresponds to
    (N0, N1, N2, N3) = (1-xi-eta-zeta, xi, eta, zeta), matching SOFA's
    `Hexa2TetraTopologicalMapping` tetrahedron orientation.
    """
    ...
    def rule(xe):
        ...
        x0, x1, x2, x3 = xe_arr
        # Constant Jacobian: edge vectors from node 0 as columns.
        J = np.column_stack([x1 - x0, x2 - x0, x3 - x0])   # (3, 3)
        detJ = np.linalg.det(J)
        vol  = abs(detJ) / 6.0
        Jinv = np.linalg.inv(J)
        # Reference gradients of (N0, N1, N2, N3) w.r.t. (xi, eta, zeta).
        ...
        dN_dy = Jinv[0, 1] * dN_dxi + Jinv[1, 1] * dN_deta + Jinv[2, 1] * dN_dzeta
        dN_dz = Jinv[0, 2] * dN_dxi + Jinv[1, 2] * dN_deta + Jinv[2, 2] * dN_dzeta
        dN_phys = np.stack([dN_dx, dN_dy, dN_dz])  # (3, 4) [d, a]
...
def tri_face_rule(n_pts=3):
    """Facet rule for a 3D boundary triangle: linear shape, 2D triangle Gauss.
    ...
    def rule(xe):
        ...
        v1 = xe_arr[1] - xe_arr[0]
        v2 = xe_arr[2] - xe_arr[0]
        area2 = np.linalg.norm(np.cross(v1, v2))   # = 2 * aire du triangle
        for (xi, eta), w_ref in zip(pts, wts):
            N = np.array([1.0 - xi - eta, xi, eta])
            coords = N @ xe_arr
            yield coords, w_ref * area2, N
    return rule

...
from fem import hex_q1_rule
from elements import element_hex
...
def build_solid_scene(rootNode, mms, element, L=1.0, E=1e6, nu=0.3,
    ...
    """Build a SOFA scene for `mms` on the 3D `element` strategy using
    HexahedronCorotationalFEMForceField.

    Returns (dofs, topology).
    """
    rootNode.addObject("RequiredPlugin", pluginName=[
        ...
        "Sofa.Component.SolidMechanics.FEM.Elastic",   # HexahedronCorotationalFEMForceField
    ...
    # ------------------------------------------------------------------
    # Corotational hexahedral FEM
    # ------------------------------------------------------------------
    # method: "large" (default) uses the polar-decomposition corotational
    # frame; "small" falls back to linear kinematics; "polar2" is an
    # alternative polar variant.  Use "large" for a genuine CR formulation.
    Solid.addObject("HexahedronCorotationalFEMForceField",
                    name="FEM",
                    template="Vec3d",
                    youngModulus=E,
                    poissonRatio=nu,
                    rotationMethod="large")
    # ------------------------------------------------------------------
...
def solve_solid(elem, mms, L, E, nu, nx, ny, nz):
    ...
    # ── AJOUT 1 : sauvegarder X avant tout ────────────────────────────
    nodes_3d_original = get_nodes_3d(L, nx, ny, nz)
...
def plot_solution_slices(stem, sol, mms, L, nx, ny, nz, label, nu):
    """Save midplane heatmaps to results/<stem>.png."""
    os.makedirs(RESULTS_DIR, exist_ok=True)
    xyz = sol.nodes[:, :3]
    mid_i, mid_j, mid_k = nx // 2, ny // 2, nz // 2

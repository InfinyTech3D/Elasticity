...
from sofa_beam2d_compression_cp import sofaRun, read_gmsh_2d
...
def analytical_far_field(x, y, q, E, nu):
    """Uniform uniaxial compression solution (plane stress), valid away
    from the clamped edge (Saint-Venant). Simple 1D-Hooke-like relation
    since sigma_yy = 0 is the *native* plane stress assumption (no extra
    (1-nu^2) factor, unlike the plane strain case)."""
    ux = -q * x / E
    uy = q * nu * y / E
    return ux, uy
...
if __name__ == "__main__":
    ...
    # --- Run FreeFEM ---
    runner = FreeFemRunner("freefem_beam2d_compression_cp.edp")
    ...
    # --- Far-field analytical check (x >= SAINT_VENANT_FACTOR * H) ---
    ...
    with open("results/comparison_beam2d_compression_cp_results.txt", 'w') as f:
    ...
    fig.suptitle("2D Beam — Compression — SOFA vs FreeFEM (plane stress)", fontsize=14)
    ...
    plt.tight_layout()
    fig.savefig("results/comparison_beam2d_compression_cp_fields.png", dpi=150)
    plt.close(fig)
"""
2D Beam Simulation - Distributed Load - Plane Strain - P1 Triangles
...
Uses the plugin's LinearSmallStrainFEMForceField with template="Vec3d" for
plane strain, exactly as validated in the 2D MMS quadratic case.
"""
import json
import os
import sys
...
def create_scene_args(rootNode, mesh_file, q, young_modulus, poisson_ratio):
    ...
    # Plane strain -> template Vec3d (z kept at 0, matches the validated MMS pattern)
    template = "Vec3d"
...
def sofaRun(mesh_file, q, young_modulus, poisson_ratio):
    ...
    u = pos_final[:, :2] - pos0[:, :2]   # keep only ux, uy (drop the z=0 plane-strain dof)
    x0 = pos0[:, :2]

    os.makedirs(RESULTS_DIR, exist_ok=True)
    out_path = os.path.join(RESULTS_DIR, "sofa_beam2d_distributed_results.txt")

"""
2D Beam - Distributed Load - Plane Strain - Comparison File

No analytical solution available for this case -> pure cross-validation
SOFA vs FreeFEM, RMS computed separately on ux and uy (as in the PR1
task list: RMS_ux(SOFA vs FF), RMS_uy(SOFA vs FF)).
"""
import json
import os
import sys
...
from sofa_beam2d_distributed import sofaRun, read_gmsh_2d
...
if __name__ == "__main__":
    ...
    # --- Run FreeFEM ---
    runner = FreeFemRunner("freefem_beam2d_distributed.edp")
    ...
    # --- Pair FreeFEM nodes onto the SOFA node ordering (same mesh, same coords) ---
    ...
    fig.suptitle("2D Beam — Distributed Load — SOFA vs FreeFEM (plane strain)", fontsize=14)
    ...
    print("  - comparison_beam2d_distributed_results.txt   (table + RMS_ux / RMS_uy)")
    print("  - comparison_beam2d_distributed_fields.png    (ux/uy fields: SOFA / FF / diff)")